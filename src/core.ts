import type {
  ...
  async function createApicoResponse<T>(
    ...
    try {
      ...
      } else if (responseType === 'text' || response.headers.get('content-type')?.includes('text/')) {
   ...
   */
  export function createApicoInstance(defaultConfig: RequestConfig = {}): ApicoInstance {
    ...
    // Core request method
    async function request<T = any>(config: RequestConfig): Promise<ApicoResponse<T>> {
      ...
      // Set up timeout
      const { controller, timeoutId } = createTimeoutController(requestConfig.timeout);
      ...
      try {
        ...
        if (['POST', 'PUT', 'PATCH'].includes(method) && requestConfig.data !== undefined) {
    ...
    async function requestSafe<T = any>(config: RequestConfig): Promise<RequestResult<T>> {
      try {
    ...
    function get<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'GET' });
    }
    
    function delete_<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'DELETE' });
    }
    
    function head<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'HEAD' });
    }
    
    function options<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'OPTIONS' });
    }
    
    function post<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'POST', data });
    }
    
    function put<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'PUT', data });
    }
    
    function patch<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<ApicoResponse<T>> {
      return request<T>({ ...config, url, method: 'PATCH', data });
    }
    
    // Safe method variants
    function getSafe<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'GET' });
    }
    
    function deleteSafe<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'DELETE' });
    }
    
    function headSafe<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'HEAD' });
    }
    
    function optionsSafe<T = any>(url: string, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'OPTIONS' });
    }
    
    function postSafe<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'POST', data });
    }
    
    function putSafe<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
      return requestSafe<T>({ ...config, url, method: 'PUT', data });
    }
    
    function patchSafe<T = any>(url: string, data?: any, config?: Omit<RequestConfig, 'url' | 'method' | 'data'>): Promise<RequestResult<T>> {
